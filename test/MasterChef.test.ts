...
const PequiReferral = artifacts.require("PequiReferral");

contract(
  "MasterChef",
  ([alice, bob, carol, referrer, treasury, dev, fee, owner]) => {
    beforeEach(async () => {
      this.zeroAddress = "0x0000000000000000000000000000000000000000";
      this.pequi = await PequiToken.new({ from: owner });
      this.referral = await PequiReferral.new({ from: owner });
      ...
      await this.referral.updateOperator(this.chef.address, true, {
        from: owner,
      });
      await this.chef.setPequiReferral(this.referral.address, { from: owner });
...
function contract(
  arg0: string,
  arg1: ([alice, bob, carol, referrer, treasury, dev, fee, owner]: [
    any,
    any,
    any,
    any,
